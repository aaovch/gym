...
export const SCHEMA_ENTITIES: SchemaEntity[] = [
	{
		...
		description: 'Корень файла тренировок. v4 валидируется при загрузке; v1–v3 мигрируют автоматически.',
		fields: [
			{ name: 'version', type: '4', description: 'Номер схемы' },
			{ name: 'revision', type: 'number', description: 'Монотонная ревизия для синхронизации' },
  ...
  "logs": [ … ]
	...
	},
	{
		...
		fields: [
			...
			{ name: 'kind', type: "'strength' | 'run' | 'jumps'", description: 'Тип нагрузки' },
			{
				name: 'movementBlocks',
		...
		example: `// runtime
{
  ...
  "date": "2026-06-10",
  "blocks": [{ "kind": "strength", "sets": [[80, 5], [80, 5]], "comment": null }],
...
// stored v4
{
  ...
  "blocks": [{
	...
	},
	{
		...
		description: 'Группа подходов с общим комментарием. На диске значения имеют именованные поля.',
		fields: [
			{
				name: 'kind',
				type: "'strength' | 'run' | 'jumps'",
				description: 'Определяет семантику пары и формат на диске'
			},
			{
				name: 'sets',
				type: 'StrengthSet[] | RunSet[] | JumpSet[]',
				description: 'weightKg/reps, durationMin/speedKmh или setCount/repsPerSet'
			},
			{ name: 'comment', type: 'string | null', description: 'Заметка к блоку', optional: true }
		],
		example: `{ "kind": "run", "sets": [{ "durationMin": 20, "speedKmh": 8.5 }] }`
	},
	{
		id: 'cycle-plan',
		title: 'CyclePlan',
		...
		description: 'План периодизации. Диапазоны дат вычисляются из сессий, а не дублируются на диске.',
		fields: [
			{ name: 'version', type: '4', description: 'Номер схемы' },
			{ name: 'revision', type: 'number', description: 'Монотонная ревизия для синхронизации' },
  ...
  "updatedAt": "…",
  "templates": [],
  "macrocycles": [{ "id": "macro-1", "label": "Сила", "mesoIds": ["meso-1"] }],
  "mesocycles": [ … ]
}`,
	...
	},
	{
		...
		fields: [
			...
			{ name: 'startDate', type: 'string', description: 'Runtime: вычисляется по датам мезо' },
			{ name: 'endDate', type: 'string', description: 'Runtime: вычисляется по датам мезо' },
	...
	},
	{
		...
		fields: [
			...
			{ name: 'startDate', type: 'string', description: 'Runtime: первая назначенная дата' },
			{ name: 'endDate', type: 'string', description: 'Runtime: последняя назначенная дата' },
	...
	},
	{
		...
		fields: [
			...
			{ name: 'indexInMeso', type: 'number', description: 'Порядковый номер в мезо (1-based)' },
...
// На диске используется та же явная форма session.`,
...
MesocyclePlan ── microcycles[], anchor1rm, templateId
       │
MacrocyclePlan ── mesoIds[] (единственная связь макро → мезо)
       │
